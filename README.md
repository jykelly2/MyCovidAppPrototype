...
2.	Recommendations for all possible cases depending on the user's health determined from daily self-check. For example, if a user has dry cough and slight fever, recommend user to filter out the air in the room and keep monitoring the temperature. If temperature stays above certain degrees for some time, go contact hospital and ask for emergency.

3.	Provide call center help as well as information about the nearest hospital (how busy it is, is testing available, drive through test etc.).

4.	Inform user through news feed as well and gather all news resources related to COVID-19. Have search or filter feature to filter out news.
